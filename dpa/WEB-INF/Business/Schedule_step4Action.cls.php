...
class Schedule_step4Action extends Action {
    /**
     *
     * @access public
     * @param array &$request
    ...
    function execute(&$actionMap,&$actionError,$request,&$response,$form,$get,$cookie, $files=array()){
        ...
        if (1==$request["rewind"]) {
          ...
          $_arr["shell_command"]   = $request["shell_command"];
        }else {
        ...
        $dbR->table_name = TABLENAME_PREF."schedule";
          ...
          // 如果是添加或更新的，则为数组，需要转成字符串
          $_arr["minute"] = implode(",", $request["minute"]);
          $_arr["hour"]   = implode(",", $request["hour"]);
          $_arr["day"]   = implode(",", $request["day"]);
          $_arr["month"]   = implode(",", $request["month"]);
          $_arr["week"]   = implode(",", $request["week"]);
        }

        // if has per,
        if ($request["per_minute"]>1 && "*"==$_arr["minute"]) {
          $_arr["minute"] = $_arr["minute"]."/".$request["per_minute"];
        }
        if ($request["per_hour"]>1 && "*"==$_arr["hour"]) {
          $_arr["hour"]   = $_arr["hour"] . "/".$request["per_hour"];
        }
