...
$dbR = new DBR();
$dbW = new DBW();
$table_name = TABLENAME_PREF."sector_mem";
$table_hq = TABLENAME_PREF."sector_hq";
$table_name_arr = array("sector_mem"=>"大盘行情分时历史记录","sector_hq"=>"大盘行情");

// 由于需要将当前抓取的行情更新到大盘行情表中去, 所以需要将旧行情数据获取到，切总共只有64不超过100条，因此一次性取出
$dbR->table_name = $table_hq;
$old_hq_arr = $dbR->getAlls();
// 获取 sec_id 的中文名称
$dbR->table_name = TABLENAME_PREF."sector";
...
function main(&$dbR, &$dbW, $tbl_name, $_G_timeout,$_G_cookie_arr=array()){
  ...
  $uni_arr = array("sec_id","trade_day","time");
...
function insertmem(&$dbR, &$dbW, &$l_mem, $mem_tbl_name,$unique_arr,$a_id,$table_name_cn,$trade_day,$add_field){
  ...
  if (!$l_r_n && !$l_r_q) {
    ...
    $data_arr["sec_id"] = $a_id;
    $data_arr["trade_day"] = $trade_day;
    $dbW->table_name = $mem_tbl_name;
    inserone($dbW, $data_arr,$unique_arr);

    // 更新或插入到行情表中去, 唯一性条件就看sec_id存在否 !!!!多一个中文名称!!
    $dbW->table_name = $table_hq;
    inser_update_hq($dbW, array_merge($data_arr,array("name_cn"=>$name_cn)),"sec_id=".$data_arr["sec_id"]);
  }else {
